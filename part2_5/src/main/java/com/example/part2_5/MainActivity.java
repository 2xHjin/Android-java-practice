...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if(view==vibrationBtn){
          /*  VibratorManager vib;
            vib = (VibratorManager) getSystemService(VIBRATOR_MANAGER_SERVICE);
            Vibrator vibrator = vib.getDefaultVibrator();

            if(Build.VERSION.SDK_INT>=26) {
                vibrator.vibrate(VibrationEffect.createOneShot(1000, 17));
            }
            */
